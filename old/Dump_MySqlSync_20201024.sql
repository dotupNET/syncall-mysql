...
BEGIN
    ...
    if not (NEW.SyncMode = '2') then
        insert into SyncLastTableChanges (TableName, LastChange, SyncOrder)
        values (NEW.TableName, UTC_TIMESTAMP(), NEW.SyncOrder);
    end if;
...
(
    ...
    SyncOrder  int default 0 not null,
    RowVersion char(36)      null,
...
create
    definer = root@`%` procedure SyncCreateClientState(IN device_id varchar(50))
BEGIN
